...
def main():
    ...
    if len(choice_list) != 0:
        ...
        df_selected = df[['Week'] + choice_list]  

        # 스트림릿에서 제공하는, 라인차트
        st.line_chart(data=df_selected.set_index('Week'))

        # 스트림릿에서 제공하는, 바차트
        st.bar_chart(df_selected.set_index('Week'))
        ...
        df_2017_2018 = df.loc[my_filter, ['Week'] + choice_list]  
        st.bar_chart(data=df_2017_2018.set_index('Week'))
        ...
        # 1.pie 차트
        chart2 = px.pie(df_prog, names= 'lang', values= 'Sum', title='각 언어별 선호도')
